    ...
    while order != 'd': # while the user is not done
        ...
        if order == 'rc':
            ...
            while error == 0: # while the file is not a good file
                file = input("(File was Incorrect)whats the name of the file: \n") # gets new file name from the user
        ...
        if order == 'w':
            file = input("whats the name of the file you want to write to(will write over whats in there): \n") # gets teh file name from the user
            error = testfile(file) # tests the file to see if it exists
            while error == 0:# while the file does not exist
                file = input("(An error was found) whats the name of the file: \n") # keeps getting a file name from the user
        ...
        order = input("What Would you like to do( a(add), s(search), p(print), r(remove), sh(shipout), rc(read cont), ra(read pack), w(write), d(done); \n") # after the action finds out what the user wants to do next
